...
13. **Preflight External Tooling**:
    ...
    - `universal_transcribe.py` re-exports every name it was split into, so `universal_transcribe.<anything>` still resolves and no caller had to change. But when a test needs to replace one, patch the module the function actually lives in (`nlm_client`, `query_execution`, `phase_validation`) -- patching the re-export does nothing, because the function resolves its own globals.

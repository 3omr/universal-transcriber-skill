...
class TestVersionChecker(unittest.TestCase):
    def test_version_string_and_parse(self):
        self.assertEqual(version_checker.get_current_version(), "1.3.0")
    ...
    def test_is_newer_version(self):
        self.assertTrue(version_checker.is_newer_version("1.4.0", "1.3.0"))
        self.assertTrue(version_checker.is_newer_version("2.0.0", "1.9.9"))
        self.assertTrue(version_checker.is_newer_version("1.3.1", "1.3.0"))
        self.assertFalse(version_checker.is_newer_version("1.3.0", "1.3.0"))
        self.assertFalse(version_checker.is_newer_version("1.2.0", "1.3.0"))
        self.assertFalse(version_checker.is_newer_version("0.9.0", "1.3.0"))
    ...
    def test_check_for_updates_with_cache(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cache_file.write_text(
                json.dumps({"latest_version": "1.4.0", "checked_at": time.time()}),
                encoding="utf-8",
            )
    ...
    def test_check_for_updates_network_fetch_and_cache(self, mock_fetch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(cached_data["latest_version"], "1.4.0")
    ...
    def test_cli_version_flags(self):
        ...
        self.assertIn("1.3.0", res_ut.stdout + res_ut.stderr)
        ...
        self.assertIn("1.3.0", res_anki.stdout + res_anki.stderr)
